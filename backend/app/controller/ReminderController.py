...
def ubah(id_reminder):  
    try:  
        ...
        reminder.reminder_time = datetime.strptime(reminder_time, '%Y-%m-%d %H:%M:%S') if reminder_time else reminder.reminder_time  
        # reminder.status = status if status else reminder.status  
